...
def train_state_estimator(memory, model, optimizer, criterion, batch_size=32):
    ...
    model.train()
    optimizer.zero_grad()
    outputs, _ = model(states.unsqueeze(1))  # 시퀀스 차원 추가
...
def simulate():
    ...
    for episode in range(MAX_EPISODES):
        state = env.reset()
        last_valid_state = state if last_valid_state is None else last_valid_state
        total_reward = 0
        video = []
        ...
        for t in range(MAX_TRY):
            ...
            train_state_estimator(memory, state_estimator, rnn_optimizer, rnn_criterion)  # RNN 학습
            ...
            # 화면 저장
            screen = env.render()
            if screen is not None and episode % 100 == 0:
                video.append(screen)
        ...
        if episode % 100 == 0:
            ...
            save_model(state_estimator, f'./output/ddqn/model/state_estimator_{episode}.pth')
